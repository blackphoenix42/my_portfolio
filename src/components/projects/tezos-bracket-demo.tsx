...
export function TezosBracketDemo() {
  ...
  const t = useTranslations("demos.tezos");
  ...
  useEffect(() => {
    ...
    // With reduced motion, fast-forward to the final round instantly.
    if (reduce) {
      setRounds((rs) => {
        const next = [...rs];
        while (next[next.length - 1]!.length > 1) {
          next.push(simulateRound(next[next.length - 1]!));
        }
        return next;
      });
      return;
  ...
  return (
    <div className="card overflow-hidden">
      <div className="border-border bg-bg-sunken/60 text-fg-subtle flex items-center justify-between border-b px-4 py-2 font-mono text-xs">
        <span>{t("title")}</span>
        <span>{t("round", { current: rounds.length, total: Math.log2(TEAMS.length) + 1 })}</span>
      </div>
      <div className="grid gap-6 p-5 lg:grid-cols-[1fr,220px]">
          ...
          {rounds.map((round, ri) => (
            <div key={ri} className="flex min-w-[110px] flex-col justify-around gap-2">
              <p className="text-fg-subtle font-mono text-[10px]">
                {ri === rounds.length - 1 && round.length === 1
                  ? t("championLabel")
                  : t("roundLabel", { n: ri + 1 })}
              </p>
              {round.map((t, ti) => (
        ...
        <div className="space-y-3">
          ...
          >
            <Trophy className="h-3.5 w-3.5" /> {t("play")}
          </button>
          <button
            type="button"
            onClick={() => {
          ...
          {champion && (
            <p className="border-accent-amber/40 bg-accent-amber/10 text-accent-amber rounded-md border p-2 text-center font-mono text-xs">
              <Trophy className="mr-1 inline h-3 w-3" aria-hidden /> {champion}
            </p>
          )}
